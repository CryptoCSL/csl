var canvas = document.getElementById("desktopGameCanvas");
var drawingSurface = canvas.getContext("2d");


function resizeGame() {
    const outerContainer = document.querySelector('.main-desktop-layout');
    const screenWidth = window.innerWidth;
    const screenHeight = window.innerHeight;

    // Original game dimensions with Title = 900 x 720
    // Original game frame 800 x600
    // Calculate scale factor
    const scaleWidth = screenWidth / 900;
    const scaleHeight = screenHeight / 720;
    ...
    outerContainer.style.height = `${scale * 720}px`;
}
