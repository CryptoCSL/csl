...
function resizeGame() {
    const outerContainer = document.querySelector('.main-mobile-layout');
    const screenWidth = window.innerWidth;
    const screenHeight = window.innerHeight;

    // Original game dimensions with buttons = 1100 x 600
    // Original game frame 800 x600
    // Calculate scale factor
    const scaleWidth = screenWidth / 1100;
    const scaleHeight = screenHeight / 600;
    ...
    outerContainer.style.width = `${scale * 1100}px`;
    outerContainer.style.height = `${scale * 600}px`;
}
